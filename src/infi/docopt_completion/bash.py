...
class BashCompletion(CompletionGenerator):
    ...
    def get_completion_path(self):
        return os.path.join(Path.home(), ".bash_completion.d")
    ...
    def create_subcommand_switch(self, cmd_name, level_num, subcommands, opts):
        ...
        subcommand_cases = '\n'.join(CASE_TEMPLATE.format(
            subcommand, cmd_name) for subcommand in subcommands)
    ...
    def create_compreply(self, param_tree):
        ...
        word_list = " ".join(param_tree.options) + " " + \
            " ".join(param_tree.subcommands.keys())
    ...
    def create_section(self, cmd_name, param_tree, option_help, level_num):
        ...
        subcommand_switch = self.create_subcommand_switch(
            cmd_name, level_num, subcommands, opts)
        res = SECTION_TEMPLATE.format(cmd_name=cmd_name,
                                      level_num=level_num,
                                      compreply=self.create_compreply(
                                          param_tree),
    ...
    def get_completion_file_content(self, cmd, param_tree, option_help):
        completion_file_inner_content = self.create_section(
            self.sanitize_name(cmd), param_tree, option_help, 1)
